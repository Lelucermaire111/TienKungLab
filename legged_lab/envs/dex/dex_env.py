...
from legged_lab.envs.dex.run_cfg import DexRunFlatEnvCfg
...
class DexEnv(VecEnv):
    def __init__(
        self,
        cfg: (DexRunFlatEnvCfg
        ),
        headless,
    ):
        self.cfg: (DexRunFlatEnvCfg
        )
    ...
    def init_buffers(self):
        ...
        # self.all_joint_ids, _ = self.robot.find_joints(
        #     name_keys=[
        #         "hip_roll_l_joint",
        #         "hip_pitch_l_joint",
        #         "hip_yaw_l_joint",
    ...
    def visualize_motion(self, time):
        ...
        # dof_pos[:, self.waist_joints_ids] = visual_motion_frame[18:21]
        dof_pos[:, self.waist_joints_ids] = torch.zeros(3,device=self.device)
        ...
        # dof_vel[:, self.waist_joints_ids] = visual_motion_frame[47:50]
        dof_vel[:, self.waist_joints_ids] = torch.zeros(3,device=self.device)
        ...
        # self.waist_dof_pos = dof_pos[:,self.waist_joints_ids]
        # self.waist_dof_vel = dof_vel[:,self.waist_joints_ids]
        return torch.cat(
            (
    ...
    def get_amp_obs_for_expert_trans(self):
        ...
        # self.waist_dof_pos = self.robot.data.joint_pos[:,self.waist_joints_ids]
        # self.waist_dof_vel = self.robot.data.joint_vel[:,self.waist_joints_ids]
